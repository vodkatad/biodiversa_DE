### vedere i pdx e gli lmh che sono false perché non hanno il pdo
...
keep_sample <- mergedx_good$sample

meda_x <- meda_x[meda_x$sample %in% keep_sample,]

##gli lmh buoni
meda_h <- meda_f
meda_h <- filter(meda_h, type == "LMH")
meda_h <- meda_h %>% mutate(sample_id_R = gsub("-2", ".2", sample_id_R))
meda_h <- meda_h %>% remove_rownames %>% column_to_rownames(var="sample_id_R")

mergedh_good <- merge(meda_h, pdo_buoni_t, by= "sample")
keep_sample_h <- mergedh_good$sample

meda_h <- meda_h[meda_h$sample %in% keep_sample,]

meda_fin <- rbind(meda_x, meda_o, meda_h)
