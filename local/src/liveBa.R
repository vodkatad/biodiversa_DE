...
mewt <- me[me$mut=="WT",]
ggplot(data=mewt, aes(x=overall, y=co_comp_N2))+geom_point()+geom_smooth(method='lm')
cor.test(mewt$overall, mewt$co_comp_N2)

cor.test(me[me$mut=="MUT",]$overall, me[me$mut=="MUT",]$co_comp_N2)
memut <- me[me$mut=="MUT",]
ggplot(data=memut, aes(x=overall, y=co_comp_N2))+geom_point()+geom_smooth(method='lm')
...
### ARID1A #######################
d <- read.table('/home/mferri/excel_geni_tcf_bcat/ARID1A_TCF_mut.tsv', sep="\t", stringsAsFactors = F, header=T)
order_dep_f <- "/mnt/cold1/snaketree/prj/DE_RNASeq/local/share/data/tcf7l2_order_def.xlsx"
order_dep <- read_xlsx(order_dep_f)
order_dep$quartile <- ntile(order_dep$co_comp_N2, 4) 
names(order_dep)[names(order_dep)=="case"] <- "smodel" 
order_dep <- as.data.frame(order_dep) 

m <- merge(d, order_dep, by.x="smodel")

ggplot(data=m, aes(x=reorder(smodel,-co_comp_N2), y=co_comp_N2, fill=mutation))+geom_col()+theme_bw(base_size = 20)+
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))+scale_fill_manual(values=c('orange1', 'green4', 'cyan', 'grey'))


cet <- read.table('/mnt/trcanmed/snaketree/prj/pdxopedia/local/share/data/treats/last_march2024/last_cet_march2024.txt', sep="\t", stringsAsFactors = F, header=T)
colnames(cet)[1] <- 'smodel'
colnames(cet)[2] <- 'cet3w'

m2 <- merge(d, cet, by.x="smodel")

ggplot(data=m2, aes(x=reorder(smodel,-cet3w), y=cet3w, fill=mutation))+geom_col()+theme_bw(base_size = 20)+
  theme(axis.text.x = element_blank())+scale_fill_manual(values=c('orange1', 'green4', 'cyan', 'grey'))
