...
krt <- "/home/mferri/Pd_high_low_krt.tsv"
krt <- read.table(krt, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
krt$sample.names <- rownames(krt)
krt$cases <- substr(rownames(krt), 1, 7)

merged2 <- merge(merged, krt, by = "cases")
...
merged3 <- merge(merged2, cris, by = "sample.names")
res <- as.data.frame(cbind(merged3$sample.names, merged3$cases, merged3$col, merged3$predict.label2, merged3$KRT_classification))
colnames(res) <- c("genealogy", "cases", "col", "CRIS", "KRT_classification")
res$genealogy <- as.character(res$genealogy)
res$cases <- as.character(res$cases)
