...
dir <- '/scratch/trcanmed/DE_RNASeq/local/share/data'
...
data <- read.table("/scratch/trcanmed/DE_RNASeq/dataset/chemio_jul23/chemiojul23_clinical_data_for_circos.tsv", sep="\t", header=TRUE, stringsAsFactors = FALSE, row.names = 1)
#data$Classification_N <- as.character(data$Classification_N)
#data$Classification_T <- as.character(data$Classification_T)
cols <- read.table("/scratch/trcanmed/DE_RNASeq/local/share/data/legenda_circos_chemiojul23.tsv", sep="\t", header=TRUE, stringsAsFactors = FALSE, comment.char = "")
cols[is.na(cols)] <- "NA"
