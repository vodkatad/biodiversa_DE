...
connector <- filter(connector, col %in%  c(uno, due))
...
merged <- merge(connector, meda, by = "ShortID")
merged <- merged[, c("sample_id_R", "ShortID", "col", "batch")]
rownames(merged) <- merged$sample_id_R
merged$sample_id_R <- NULL
names(merged)[names(merged) == "ShortID"] <- "model"
