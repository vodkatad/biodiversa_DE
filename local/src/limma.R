...
lmolmx <- "/scratch/trcanmed/DE_RNASeq/dataset/Class_comparison_biobanca/Clustering_Cutoff0.05_LFC0.584/type_cutoff0.05-LMO_BASALE.vs.LMH.deseq2.tsv"
ox <- read.table(lmolmx, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

singlimma <- rownames(top.table[top.table$adj.P.Val<0.05,])
...
merged_genes <- as.data.frame(merged[merged$log2FoldChange < -10,]$Row.names)
