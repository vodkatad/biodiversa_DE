### selection of PD for keratin high and low
...
#krt_all <- krt$genes[krt$both == TRUE]
...
## NO CONNECTOR THIS TIME, DOING THESE CLASSIFICATION ON PD OUTSIDE CONNECTOR
...
merged2 <- merge(vsd_subset, recist, by = "case")
merged2 <- merged2 %>% filter(classification == "PD")
...
merged2$perc <- NULL
write.table(merged2, file = "/home/mferri/OUTCON_PD_10_krt.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)

## preparo un df per i quartili e calcolo i quartili
samples_krt <- merged2
# quartiles_krt <- data.frame(matrix(ncol = 3, nrow = 10))
# #quartiles_krt <- data.frame(matrix(ncol = 3, nrow = 4))
...
#final_eg <- data.frame(matrix(ncol = 4, nrow = 18))

quartiles_krt <- "/home/mferri/quartiles_krt_Ac.tsv"
quartiles_krt <- read.table(quartiles_krt, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
colnames(quartiles_krt) <- c("genes", "firstQ_25%", "thirdQ_75%")


final_eg <- data.frame(matrix(ncol = 10, nrow = 139))
...
classification <- as.data.frame(apply(final2, 1, check_high_low))
colnames(classification) <- c("KRT_classification")

write.table(classification, file = "/home/mferri/OUTCON_Pd_high_low_krt.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)
write.table(final2, file = "/home/mferri/OUTCON_allchrts_Pd_high_low_krt.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)
