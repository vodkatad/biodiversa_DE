...
km <- read.table('/mnt/trcanmed/snaketree/prj/pdxopedia/local/share/data/turin_n613_xeno_qn_genefilter_opt2_KMcalls_13052021.txt', sep="\t", header=T)
head(km)
...
ggplot(pld, aes(x = factor(KM.prediction), y = perc*100, fill = factor(class))) +
  geom_bar(stat="identity", width = 0.7) +
  labs(x = "KMpred", y = "percent", fill = "Cetuxi 3Wd%") +
  theme_bw(base_size = 14)+scale_fill_manual(values=c('red','darkblue','grey'))
