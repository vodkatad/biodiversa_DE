...
striplots <- function(gene, dds) {
    ...
    e2 + geom_point(  aes_string(shape = what, color = 'sample'),  size = 3) +theme_bw()+scale_y_continuous(trans='log10')+labs(color = "Xeno", x="Xeno", shape="Xeno", y="Log10(nreads)")+geom_line(aes_string(x="treat",y="count", color="sample", group="group"))+ggtitle(gene)
