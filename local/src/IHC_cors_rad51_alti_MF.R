...
rad51_basal <- read.table(snakemake@input[["rad51_basal_df"]], sep="\t")
rad51_irino <- read.table(snakemake@input[["rad51_irino_df"]], sep="\t")
pold1_basal <- read.table(snakemake@input[["pold1_basal_df"]], sep="\t")
w3 <- read.table(snakemake@input[["w3_df"]], sep="\t", row.names=1)


#rad51_basal <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/avgall_RAD51_NT.tsv', sep="\t")
#rad51_irino <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/avgall_RAD51_irino.tsv', sep="\t")
#pold1_basal <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/avgall_POLD1.tsv', sep="\t")

#w3 <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/w3_waterfall.tsv', sep="\t", row.names=1)
colnames(w3) <- 'dvw3'
...
luraghi <- read.table(snakemake@input[["luraghi_df"]], sep="\t")
...
H2AX_basal <- read.table(snakemake@input[["H2AX_basal_df"]], sep="\t")
H2AX_irino <- read.table(snakemake@input[["H2AX_irino_df"]], sep="\t")

#H2AX_basal <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/avgall_H2AX_NT.tsv', sep="\t")
#H2AX_irino <- read.table('/mnt/trcanmed/snaketree/prj/strata/dataset/figures/avgall_H2AX_irino.tsv', sep="\t")
...
rad5110 <- m5
for (i in rownames(rad5110)) {
  if(rad5110[i, "RAD51_irino"] > 10) {
    rad5110[i, "keep"] <- "yes"
  } else {
    rad5110[i, "keep"] <- "no"
  }
...
rad5110 <- rad5110 %>% filter(!keep == "no")

result <- snakemake@output[["res"]]

write.table(rad5110, result, quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)
