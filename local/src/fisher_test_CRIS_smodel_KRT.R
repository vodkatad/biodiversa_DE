...
cris <- "/home/mviviani/misc_tmp/CONNECTOR/Jun2022/June2022_cluster-prepared.tsv"
cris <- read.table(cris, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)

merged <- merge(krt, cris, by = "model")
res <- as.data.frame(cbind(merged$model, merged$cluster, merged$cris, merged$KRT_classification))
colnames(res) <- c("model", "col", "CRIS", "KRT_classification")
...
str <- c("CRC0028", "CRC0330", "CRC1473")
res <- res %>% filter(!model %in% str)
