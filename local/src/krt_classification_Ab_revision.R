### selection of Ab_PD for keratin high and low
...
## merge tra connector e recist per filtrare Ab e PD
...
connector2 <- connector2 %>% filter(Cluster == "Ab")
...
quartiles_krt <- data.frame(matrix(ncol = 3, nrow = 14))
...
final <- data.frame(matrix(ncol = 14, nrow = 35))
...
final_eg <- data.frame(matrix(ncol = 14, nrow = 35))
...
write.table(final2, file = "/home/mferri/Ab_high_low_krt_revision.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)
