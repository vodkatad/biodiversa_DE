...
connector <- filter(connector, !col %in%  c("Aa", "Ab"))
connector$group <- NA
for(i in seq(length(connector$ShortID))) {
  if (connector[i,"col"] == "Ac") {
    connector[i, "group"] <- "Acde"
  } else if (connector[i,"col"] == "Ad"){
    connector[i, "group"] <- "Acde"
  } else if (connector[i,"col"] == "Ae") {
    connector[i, "group"] <- "Acde"
  } else {
    connector[i, "group"] <- "B_C"
  }
}
...
merged <- merged[, c("sample_id_R", "model", "group", "batch")]
