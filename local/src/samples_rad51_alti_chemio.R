...
keep_f <- snakemake@input[["casekeep"]]
...
#keep_f <- "/scratch/trcanmed/DE_RNASeq/dataset/deg_rad51_chemio/onlyfordeg.tsv"
keep <- read.table(keep_f, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
keep <- rownames(keep)

meda_fin <- meda_fin %>% filter(model %in% keep)
