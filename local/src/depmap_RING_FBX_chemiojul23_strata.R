...
##data <- read.table('/mnt/trcanmed/snaketree/prj/strata/local/share/data/POLD1_logTPMpc1_23Q2_Irinotecan_IC50_GDSC2_010823.csv', sep=',', header=TRUE)
data_f <- snakemake@input[["FBX_POLD"]]
...
#data_f <- "/mnt/trcanmed/snaketree/prj/strata/local/share/data/FBH1_logTPM_23Q2_RING1_log2TPM_Expression.csv"
...
colnames(data)[2] <- 'RING1_log2(TPM+1)'
colnames(data)[3] <- 'FBXO18_log2(TPM+1)'

sink(log_f, append=TRUE)
cor.test(data$`RING1_log2(TPM+1)`, data$`FBXO18_log2(TPM+1)`)
sink()

y_breaks <- guess_ticks(data$`RING1_log2(TPM+1)`, fixed_min=1, fixed_max=8)
x_breaks <- guess_ticks(data$`FBXO18_log2(TPM+1)`, fixed_min=1, fixed_max=8)

p <- ggplot(data, aes(x=`RING1_log2(TPM+1)`, y=`FBXO18_log2(TPM+1)`)) +  geom_point(size=1) + geom_smooth(method='lm', size=1)+ # ratio is ... 1 becomes 0.939
  unmute_theme+theme(legend.position="none") + xlab('RING1_log2(TPM+1)')+ylab('FBXO18_log2(TPM+1)')+
