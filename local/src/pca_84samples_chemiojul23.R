...
metadata$type <- metadata$tertile
metadata$tertile <- NULL

#1Q responder
#3Q non responder
#2Q grigi

metadata$resp <- ifelse(
  metadata$type == "1_Q", "responder_1Q",
  ifelse(metadata$type == "2_Q", "grey", "non_responder_3Q")
)

metadata$type <- metadata$resp
metadata$resp <- NULL
...
p <- p + scale_color_manual(values =  c("grey",rgb(red=165,green=0,blue=25, max = 255), rgb(30, 85, 130, max = 255)))
p
