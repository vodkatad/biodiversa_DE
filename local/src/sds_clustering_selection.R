...
expr_lmh <- expr_selected

expr_lmh <- expr_lmh[, colnames(expr_lmh) %in% rownames(lmh_good)]
...
sds_lmh <- sd_analysis(expr_lmh)
sds_lmh <- tibble::rownames_to_column(sds_lmh, "genes")
...
merged_lmo_lmx <- merge(sds_lmo, sds_lmx, by = "genes")

sds_merged <- merge(merged_lmo_lmx, sds_lmh, by = "genes")
