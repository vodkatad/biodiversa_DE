...
d <- read.table('/scratch/trcanmed/DE_RNASeq/dataset/Class_comparison_biobanca/go_gsea_LFC0.584_cutoff0.05.tsv', sep="\t", header=T, stringsAsFactors = FALSE)

dgsea <- d[d$typenrich=="GSEA",]
dgsea <- dgsea[,c(1,seq(7,ncol(dgsea)))]

# pheatmap of padj TODO
dgsea <- dgsea[apply(dgsea[, grepl('padj', colnames(dgsea))], 1, function(x) {!any(is.na(x))}),]

dgsea$sign <- ifelse(apply(dgsea[, grepl('padj', colnames(dgsea))], 1, function(x) {any(x<0.1)}), 15, 16) 
#http://www.sthda.com/english/wiki/r-plot-pch-symbols-the-different-point-shapes-available-in-r

dgsea$minpadj <- apply(dgsea[, grepl('padj', colnames(dgsea))], 1, function(x) {min(x)})
dgsea$rankpadj <- rank(-dgsea$minpadj)
dgsea <- dgsea[order(dgsea$rankpadj),]

last <- NA
rank <- 1
dgsea$rankint <- seq(NA, nrow(dgsea))
for (i in seq(1, nrow(dgsea))) {
  if (is.na(last)) {
    last <- dgsea[i, 'rankpadj']
    dgsea$rankint[i] <- rank
  } else {
    if (last != dgsea[i, 'rankpadj']) {
      rank <- rank +1
      dgsea$rankint[i] <- rank
    } else {
      dgsea$rankint[i] <- rank
...
desc <- sapply(strsplit(dgsea$Description, ' '), function(x) {x[[1]]})
desc <- gsub('HALLMARK_','', desc)
dgsea$Description <- desc

allcolors <- colorRampPalette(c("lightgrey", "black"))
colors <- allcolors(length(unique(dgsea$rankint)))
#colors <- c("#999999", "#E69F00")
colors <- colors[dgsea$rankint]
#dgsea$p53 <- ifelse(grepl('CHOLESTEROL_HOMEOSTASIS', dgsea$Description),1,2)
#colors <- colors[dgsea$p53]

#mat <- dgsea[,grepl('NES', colnames(dgsea))]
#x <- mat[,1]
#y <- mat[,2]
#z <- mat[,3]
scatterplot3d(dgsea[,grepl('NES', colnames(dgsea))], grid=FALSE, box=FALSE, pch=dgsea$sign, color=colors)
# 3. Add grids
addgrids3d(dgsea[,grepl('NES', colnames(dgsea))], grid = c("xy", "xz", "yz"))


ggplot(data=dgsea, aes(x=NES_LMO.LMH,y=NES_LMO.LMX, color=p53))+geom_point()

fig <- plot_ly(dgsea, x = ~NES_LMO.LMH, y = ~NES_LMX.LMH, z = ~NES_LMO.LMX, color = ~-minpadj,text = ~Description)
