### selection of AC_PD for keratin high and low
...
krt <- "/home/mferri/significant_krt_Ab_revision.tsv"
krt <- read.table(krt, quote = "", sep = "\t", header = TRUE, stringsAsFactors = FALSE)
krtba <- (krt$genes)[krt$sign_Ab.vs.Ba == TRUE]
krtbb <- (krt$genes)[krt$sign_Ab.vs.Bb == TRUE]
krt_all <- unique(c(krtba, krtbb))

## carico connector e recist
connector <- "/scratch/trcanmed/connector/local/share/data/Models.RDs"
connector <- readRDS(connector)
...
connector2 <- as.data.frame(cbind(merged$ShortID, merged$Cluster, merged$classification))
colnames(connector2) <- c("case", "Cluster", "classification")
connector2 <- connector2 %>% filter(Cluster == "Ab")   ############## Perchè qui c'è Aa? ?
...
##decido di mantenere il genealogy così può servire in futuro per dare a fra il campione giusto  --EG Perfetto :)
...
#write.table(merged2, file = "/home/mferri/Ab_10_krt_revision.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)

## preparo un df per i quartili e calcolo i quartili
samples_krt <- merged2
samples_krt$Cluster <- NULL
quartiles_krt <- data.frame(matrix(ncol=3, nrow = ncol(samples_krt)))
...
write.table(quartiles_krt, file = "/scratch/trcanmed/connector/local/share/data/revision_Ab_quartiles.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)

# EG fin qui tutto ok!

## preparo un df che sia identico a sample_krt solo vuoto per inserire se il valore exp
## è low del primo Q o high del terzo Q
final <- data.frame(matrix(ncol=ncol(samples_krt), nrow = nrow(samples_krt)))
...
final_eg <- data.frame(matrix(ncol=ncol(samples_krt), nrow = nrow(samples_krt)))
...
# Facciamo qualche check a campione: CRC0382LMX0A02002TUMR02000 è dato come high per KRT80, low per la 83 e middle per la 16...
# > samples_krt[rownames(samples_krt)=="CRC0382LMX0A02002TUMR02000", c('KRT80', 'KRT83', 'KRT16')]
# KRT80    KRT83    KRT16
# CRC0382LMX0A02002TUMR02000 10.33154 6.536092 6.804328
# > quartiles_krt[rownames(quartiles_krt) %in%  c('KRT80', 'KRT83', 'KRT16'),]
# genes firstQ_25% thirdQ_75%
# KRT80 KRT80   8.947113   9.447786
# KRT83 KRT83   6.577779   6.687612
# KRT16 KRT16   6.372264   6.835221

# mi sembra tutto corretto, essendo un loop 'normale' senza casi particolari all'inizio o alla fine non faccio altri check perchè
# tutti i campioni e le krt dovrebbero essere gestiti in maniera uniforme, se corretta per uno corretta per tutti
...
check_high_low <- function(info) {
  collapsed <- unique(info)
  if ((length(collapsed) == 2 && c('high', 'middle') %in% collapsed) || (length(collapsed) == 1 && c('high') %in% collapsed) ){
    return("KRT_high")
  } else if ((length(collapsed) == 2 && c('low', 'middle') %in% collapsed) || (length(collapsed) == 1 && c('low') %in% collapsed)) {
    return("KRT_low")
  } else {
    return('KRT_ignavo')
  }
...
classification2 <- apply(final2, 1, check_high_low)
# EG: Tutti ignavi ma su Ac me lo aspettavol ma prova del 9 sarà vedere se risulta high qualche Ba o Bb...io alla riga 47 prima di fare il filtro per Ac
# mi salverei un df di tutti i pdf per poi classificare lui nel for magno

#write.table(final2, file = "/home/mferri/Ab_high_low_krt_revision_revEG.tsv", quote = FALSE, sep = "\t", col.names = TRUE, row.names = TRUE)


### classifichiamo tutti i PD
connector2 <- as.data.frame(cbind(merged$ShortID, merged$Cluster, merged$classification))
colnames(connector2) <- c("case", "Cluster", "classification")
connector2 <- connector2 %>% filter(classification == "PD")
connector_cases <- connector2$case
...
classificationPD <- apply(final_eg, 1, check_all_high_low)


classificationPD2 <- apply(final_eg, 1, check_high_low)

table(classificationPD2)

wanted <- read.table('/home/egrassi/krt_sel')
for (x in wanted$V1) {
  ffi <-grep(x, names(classificationPD2))
  if (length(ffi) >= 1) {
    #print(x)
    #print(ffi)
    print(classificationPD2[ffi])
  } 
}
